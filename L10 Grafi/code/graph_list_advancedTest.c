...
bool isPresent(Listnode list, int element) {
     Listnode temp = list;
     while (temp != NULL) {
          if (temp -> v == element)
               return true;
...
Listnode arco(Listnode list, int k) {
     ...
     if (!head)
          return p;
     if (!isPresent(list, p->v)) {         // Aggiungo solo se non è già presente l'arco per evitare ridondanza
...
void graph_edgeinsert ( Graph g, int v, int w ) {
     // devo mettere w nella lista di v, e v nella lista di w
     g -> A[v] = arco( g->A[v], w);
     g -> A[w] = arco( g->A[w], v);
     if ( isPresent(g->A[v], w) && isPresent(g->A[w], v) ) {
          // Se l'arco è già presente in entrambi i nodi, non aumento il numero di archi
          return;
     }
     g->m++;
}
...
Graph gen(int p, int n) {
     ...
     for (int i = 0; i < g->n; i++) {
          for (int j=0; j < g->n; j++) {
               if ( (rand() % p) < p) {
                    // Inserisci arco, ma controlli anche che non sia già stato inserito per evitare casini
...
int main(void) {
     ...
     printf("---Stampa BFS (Visita in ampiezza)---\n");
     bfs(g, 0);
     ...
     Graph gRand = gen(0.5, 5);      // Argomenti: p , n
     graph_print(gRand);
     bfs(gRand, 0);
}