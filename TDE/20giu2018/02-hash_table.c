...
Per memorizzare gli elementi che, tramite la funzione di hash, finiscono nella cella dizionario[valueHash].
 
2. Se t è una variabile di tipo Dict, cosa rappresenta t -> array[3] ?
 
INSERIRE LA RISPOSTA
Rappresenta la quarta cella del dizionario, nella quale finiranno gli elementi corrispondenti al valore di hash della cella.
 
3. Cosa succede se si tenta di inserire nella tabella di hash
un Item con una chiave già presente nella tabella?
 
INSERIRE LA RISPOSTA
Viene inserito nella lista corrispondente al valore di hash ad esso associato.
 
4. Cosa succede se si prova a cercare nel dizionario una chiave che non c'è?
 
INSERIRE LA RISPOSTA
Questo provoca un errore, in quanto verrebbe restituito NULL dalla funzione lookup del dizionario; viene poi moltiplicato il valore per 2 (che è nullo), causando un errore.
